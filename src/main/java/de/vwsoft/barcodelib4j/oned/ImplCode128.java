 ...
 * <p>
 * Code 128 is a widely used high-density barcode symbology that encodes all 128
 * ASCII characters,
 * covering:
 * <ul>
 ...
 * <code>!&quot;#$%&amp;'()*+,-./:;&lt;=&gt;?@[\]^_`{|}~</code></li>
 * </ul>
 * In addition, Function Codes (FNC1 to FNC4) can be used within the provided
 * input to meet
 * specific application requirements.
 * <p>
 * Code 128 uses an internal checksum included within the barcode symbol but not
 * displayed in
 * plaintext representation.
 */
public class ImplCode128 extends Barcode {

  // FNC markers. To better distinguish between invalid characters outside the
  // ASCII range 0-127
  // and FNC markers within the input, we assign 'extra high' values to the
  // latter.
  ...
  CharSequence encode() {
    ...
    while (idx < len) {
      int newCodeSet = myCodeSet == CODESET_ALL ? chooseCodeSet(myContent, idx, codeSet) : myCodeSet;

      int barsIdx;
      if (newCodeSet == codeSet) {
        barsIdx = encodeCharacter(myContent.charAt(idx), codeSet, idx);
        if (codeSet == CODESET_C)
          idx++;
        idx++;
      } else {
        barsIdx = resolveCodeSetSwitch(codeSet, newCodeSet);
        codeSet = newCodeSet;
      }
  ...
  private int encodeCharacter(char c, int codeSet, int idx) {
    switch (c) {
      case FNC1:
        return CODE_FNC_1;
      case FNC2:
      ...
      case FNC3:
        return CODE_FNC_3;
      case FNC4:
        return codeSet == CODESET_A ? CODE_FNC_4_A : CODE_FNC_4_B;
      default:
        switch (codeSet) {
          case CODESET_A:
            int barsIdx = c - ' ';
            return barsIdx < 0 ? barsIdx + '`' : barsIdx;
          case CODESET_B:
            return c - ' ';
          default: // CODESET_C
            return Integer.parseInt(myContent.substring(idx, idx + 2));
        }
    }
  }

  private static int resolveCodeSetSwitch(int codeSet, int newCodeSet) {
    if (codeSet == 0) {
      switch (newCodeSet) {
        case CODESET_A:
          return CODE_START_A;
   ...
   * <p>
   * Validates the provided content to ensure that it contains only ASCII
   * characters,
   ...
   *
   * @param content                the content to be encoded in the Code 128
   *                               barcode
   * @param autoComplete           has no function in this method implementation
   * @param appendOptionalChecksum has no function, as Code 128 uses a fixed
   *                               internal checksum
   *                               which is not optional
   * @throws BarcodeException if the content is empty, or contains non-ASCII
   *                          characters
   */
  @Override
